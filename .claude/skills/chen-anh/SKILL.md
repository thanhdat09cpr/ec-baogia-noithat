...
Ảnh nằm ở cột **D (MINH HỌA)**; mỗi hạng mục 1 ảnh, cùng dòng với hạng mục đó.
...
4. **Build lại** (ảnh tự chèn vào cột MINH HỌA):
   ...
   ```
   Build đọc `<MaPhong>.anh.csv`, tự crop + fit + nới cột D + set cao dòng. Báo "Đã chèn N ảnh".
...
- Kích thước ô ảnh & bề rộng cột D chỉnh trong `lib_boq.py` (`MINHHOA_BOX`, `MINHHOA_COL_CHARS`).
